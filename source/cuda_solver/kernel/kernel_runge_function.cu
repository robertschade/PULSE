...
CUDA_GLOBAL void PC3::Kernel::runge_func_kernel_scalar( int i, real_number t, Device::Pointers dev_ptrs, System::Parameters p,
    ...
    PC3::Hamilton::scalar( hamilton, io.in_wf_plus, i, row, col, p.N, p.periodic );
    
    const auto in_wf = io.in_wf_plus[i];
    const auto in_rv = io.in_rv_plus[i];
    const auto potential = dev_ptrs.potential_plus[i];
    const auto pump = dev_ptrs.pump_plus[i];
    complex_number result_wf = p.minus_i_over_h_bar_s * ( p.m_eff_scaled * hamilton );
    const real_number in_psi_norm = CUDA::abs2( in_wf );
    result_wf += p.minus_i_over_h_bar_s * (potential + p.g_c * in_psi_norm + p.g_r * in_rv) * in_wf;
    result_wf += -0.5 * (p.R * in_rv - p.gamma_c) * in_wf;
    io.out_wf_plus[i] = result_wf;
    io.out_rv_plus[i] = pump - ( p.gamma_r + p.R * in_psi_norm ) * in_rv;
}