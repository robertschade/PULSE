...
CUDA_GLOBAL void PC3::Kernel::RK4::runge_sum_to_final( int i, Device::Pointers dev_ptrs, System::Parameters p, bool use_te_tm_splitting ) {
    ...
    dev_ptrs.buffer_wavefunction_plus[i] = dev_ptrs.wavefunction_plus[i] + p.dt / 6.0 * ( dev_ptrs.k1_wavefunction_plus[i] + 2.0 * dev_ptrs.k2_wavefunction_plus[i] + 2.0 * dev_ptrs.k3_wavefunction_plus[i] + dev_ptrs.k4_wavefunction_plus[i] );
    dev_ptrs.buffer_reservoir_plus[i] = dev_ptrs.reservoir_plus[i] + p.dt / 6.0 * ( dev_ptrs.k1_reservoir_plus[i] + 2.0 * dev_ptrs.k2_reservoir_plus[i] + 2.0 * dev_ptrs.k3_reservoir_plus[i] + dev_ptrs.k4_reservoir_plus[i] );
    if ( not use_te_tm_splitting ) 
        return;
    dev_ptrs.buffer_wavefunction_minus[i] = dev_ptrs.wavefunction_minus[i] + p.dt / 6.0 * ( dev_ptrs.k1_wavefunction_minus[i] + 2.0 * dev_ptrs.k2_wavefunction_minus[i] + 2.0 * dev_ptrs.k3_wavefunction_minus[i] + dev_ptrs.k4_wavefunction_minus[i] );
    dev_ptrs.buffer_reservoir_minus[i] = dev_ptrs.reservoir_minus[i] + p.dt / 6.0 * ( dev_ptrs.k1_reservoir_minus[i] + 2.0 * dev_ptrs.k2_reservoir_minus[i] + 2.0 * dev_ptrs.k3_reservoir_minus[i] + dev_ptrs.k4_reservoir_minus[i] );
}