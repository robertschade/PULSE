...
void PC3::Solver::iterateFixedTimestepRungeKutta4( dim3 block_size, dim3 grid_size ) {
    ...
    const int localblock_x=4;
    const int localblock_y=4;
    const int halo_x=4;
    const int halo_y=4;
    ...
    if(!matrix.ld_init){
      ...
      for(int ib=0;ib<localblock_x*localblock_y;ib++)
      {
        ...
        for(int igy=0;igy<local_N_y_halo;igy++)
        {
          for(int igx=0;igx<local_N_x_halo;igx++)
          {
            ...
            if((ig2xg<halo_x or ig2x>=system.p.N_x+halo_x) and not system.p.periodic_boundary_x){
              ig2=-1;
            }
            if((ig2yg<halo_y or igy>=system.p.N_y+halo_y) and not system.p.periodic_boundary_y){
              ig2=-1;
            }
      //      std::cout << ib << " " << " " << bx << " " << by << " x="  << igx << " y=" << igy  << " x2=" << ig2x  << " y2=" <<ig2y  << " ig=" << ig  << " ig2=" << ig2 << std::endl;
      ...
      for(int ib=0;ib<localblock_x*localblock_y;ib++)
      {
        for(int igy=0;igy<local_N_y;igy++)
        {
          for(int igx=0;igx<local_N_x_halo;igx++)
          {
            int ig=igy*local_N_x_halo+igx;
    ...
    for(int ib=0;ib<localblock_x*localblock_y;ib++)
    {
      ...
      gp_scalar2(local_N_x_halo,local_N_y_halo,system.p.t,system.p.m2_over_dx2_p_dy2,system.p.one_over_dx2,system.p.one_over_dy2,system.p.minus_i_over_h_bar_s.imag(),system.p.g_c,system.p.g_r,system.p.R,system.p.gamma_c,system.p.gamma_r,system.p.m_eff_scaled,
      ...
      gp_scalar2(local_N_x_halo,local_N_y_halo,system.p.t+0.5*system.p.dt,system.p.m2_over_dx2_p_dy2,system.p.one_over_dx2,system.p.one_over_dy2,system.p.minus_i_over_h_bar_s.imag(),system.p.g_c,system.p.g_r,system.p.R,system.p.gamma_c,system.p.gamma_r,system.p.m_eff_scaled,
      ...
      gp_scalar2(local_N_x_halo,local_N_y_halo,system.p.t+system.p.dt,system.p.m2_over_dx2_p_dy2,system.p.one_over_dx2,system.p.one_over_dy2,system.p.minus_i_over_h_bar_s.imag(),system.p.g_c,system.p.g_r,system.p.R,system.p.gamma_c,system.p.gamma_r,system.p.m_eff_scaled,
          matrix.ld[BUFFER_WAVEFUNCTION_PLUS_REAL][ib],
          matrix.ld[BUFFER_WAVEFUNCTION_PLUS_IMAG][ib],
          matrix.ld[BUFFER_RESERVOIR_PLUS_REAL][ib],
          matrix.ld[BUFFER_RESERVOIR_PLUS_IMAG][ib],
...
void PC3::Solver::sum_block_1(int N,Type::real* __restrict A,Type::real alpha,Type::real* __restrict B,Type::real* __restrict C){
  #pragma omp simd
  for(int i=0;i<N;i++)
  {
    C[i]=A[i]+alpha*B[i];
...
void PC3::Solver::sum_block_4(int N,Type::real* __restrict A,Type::real alpha1,Type::real* __restrict B1,Type::real alpha2,Type::real* __restrict B2,Type::real alpha3,Type::real* __restrict B3,Type::real alpha4,Type::real* __restrict B4,Type::real* __restrict C){
  #pragma omp simd
  for(int i=0;i<N;i++)
  {
